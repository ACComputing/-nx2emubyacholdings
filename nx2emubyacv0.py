...
class _PyPagedMemory:
    """Sparse 64-bit virtual address space, lazily backed by 4 KiB pages (fallback)."""
...
# =============================================================================
def _py_lz4_block_decompress(src: bytes, uncompressed_size: int) -> bytes:
...
# Prefer Cython nx2emu_cy.mem when built (python setup_nx2emu_cy.py build_ext --inplace).
NX2EMU_CYTHON_MEM = False
try:
    from nx2emu_cy.mem import PagedMemory as PagedMemory
    from nx2emu_cy.mem import lz4_block_decompress as lz4_block_decompress
    NX2EMU_CYTHON_MEM = True
except ImportError:
    PagedMemory = _PyPagedMemory
    lz4_block_decompress = _py_lz4_block_decompress


# Phase 2 (optional): AArch64 hot-path in Cython — stub module nx2emu_cy/hotpath.pyx
# (not wired into CpuManager; shared Cython memory above benefits all BACKENDS keys.)
...
class CpuManager:
    ...
    def set_quantum(self, q: int) -> None:
        self.quantum = max(256, min(65536, int(q)))
...
class RyujinxStyleMain(tk.Tk):
    def __init__(self, emu: NX2Emu, display: DisplayWindow) -> None:
        ...
        if NX2EMU_CYTHON_MEM:
            _EmuLog.ok("nx2emu_cy: Cython PagedMemory + LZ4 loaded")
        else:
            _EmuLog.info("nx2emu_cy: not built — using Python memory/LZ4 "
                         "(pip install cython && python setup_nx2emu_cy.py build_ext --inplace)")
